...
if (isset($_POST['save_picture'])) {
    ...
    if (!empty($_POST["CorreoElectronico"])) {
        ...
        while ($usuario = $resultado->fetch(pdo::FETCH_ASSOC)) {
            if ($usuario["Correo"] != $_POST["CorreoElectronico"]) {
                ...
                echo '<script type="text/javascript">alert("Ese Correo ya Existe, Intente con Otro.");window.location.href="register.php";</script>';
                exit();
            }
        }
    }
    ...
    $sql = "INSERT INTO `persona`(`Nombres`, `Apellidos`, `Genero`, `FechaNacimiento`, `EstadoCivil`,FotoPerfil,`Direccion`,"
            ...
            . " `CodTipoPersona`, `FechaModicicaion`) VALUES ('$reg_Nombres','$reg_apellidos',$reg_genero,STR_TO_DATE('$reg_fecha_nacimiento','%d/%m/%Y'),$reg_estado_civil,'$_fotoPerfil_name','$reg_direccion','$reg_num_telefono','$reg_num_Celular',$reg_Pais,'$reg_TipoIdentidad','$reg_num_identidad',$reg_tipo_sangre,'3',now())";
